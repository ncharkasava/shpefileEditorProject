...
# Function to pull a single Polygon or LineString object
...
# Set org feature attribute function deals with each OGR
# data type in turn, processing the string representation
# of the attribute value and calling the appropriate SetField()
# method to set the field's value. 
...
# This function calculates the distance, in map units, of a given linear distance measured in meters.
# It calculates the lat/long coordinates for four points directly
# north, south, east, and west of the starting location and the
# given number of meters away from that point. This function allows to use the DWithin() 
# spatial query to identify all features close to the clicked-on location.
...
# GeoModelAdmin is used to generate an appropriate map widget for
# editing the type of geometry stored in the current shapefile.
# The get_map_form() function creates and returns a new django.forms.Form 
# subclass with the appropriate widget type used to edit this particular
# shapefile's features. Note that the get_map_form() function returns
# the MapForm class, rather than an instance of that class
# The returned class is used after to to create the appropriate
# MapForm instances as we need them.
