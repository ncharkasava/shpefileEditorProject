...
urlpatterns = patterns('shapeEditor.editor.views',
	...
	url(r'^import$', 'import_shapefile'),
        ...
        url(r'^edit_attributevalue/(?P<shapefile_id>\d+)/' +
            r'(?P<attributevalue_id>\d+)$', 'edit_attributevalue'),
        url(r'^add_attributevalue/(?P<shapefile_id>\d+)/(?P<feature_id>\d+)$', 'add_attributevalue'),
